;;; init-gtags.el --- summary -*- coding: utf-8; lexical-binding: t; -*-
...
(defun gtags-ext-add-gtagslibpath (libdir &optional del)
  "Add external library directory LIBDIR to environment variable GTAGSLIBPATH.
Gtags will can that directory if needed.  \\[C-u M-x]
add-gtagslibpath will remove the directory from GTAGSLIBPATH."
  (interactive "Directory containing GTAGS:\nP")
  (let (sl)
	(when (not
	...
	(setq sl (split-string (if (getenv "GTAGSLIBPATH")
                               (getenv "GTAGSLIBPATH")
                             "")
                           ":" t))
	(if del
