      ...
      (let ((pattern (nth i flymake-err-line-patterns)))
        (let ((regexp (nth 0 pattern))
          ...
          (when (string-match regexp text)
            (let ((file (and file-idx (match-string file-idx text)))
                  (line (and line-idx (match-string line-idx text)))
                  (col (and col-idx (match-string col-idx text)))
                  (err-text (and err-text-idx (match-string err-text-idx text)))))
            (when lazyflymake-debug
...
(defun lazyflymake-current-file-p (file)
  ...
  (let ((algorithm lazyflymake-file-match-algorithm))
    (cond ((or (not buffer-file-name) (not file))
           (null algorithm))
          (t
           (or (and (string= algorithm "strict")
...
(defun lazyflymake-extract-errors (output)
  ...
  (cond ((and (version< "25" emacs-version) (eq major-mode 'emacs-lisp-mode))
         ...
         (let ((lines (split-string output "[\r\n]+"  t "[ \t]+"))
           ...
           (dolist (l lines)
             (let ((err (lazyflymake-parse-err-line l)))
               (when lazyflymake-debug (message "err=%s" err))
...
(defun lazyflymake-show-errors (errors)
  ...
  (when lazyflymake-debug
    (message "lazyflymake-show-errors called"))
...
(defun lazyflymake-list-errors ()
  ...
  (interactive)
  (if lazyflymake-flymake-mode-on
      (lazyflymake-sdk-hint)
    (if-let ((ovs (lazyflymake-sdk-valid-overlays lazyflymake-overlays)))
        (let* ((cands (mapcar (lambda (o)
                                (cons (lazyflymake-overlay-to-string o) o))
                              ovs))
               (err (completing-read "Go to: " cands)))
          (when err
            (lazyflymake-goto-overlay-center
             (cdr (assoc err cands)))))
...
Return the running process."
  (let ((proc
         (apply 'start-file-process lazyflymake-process-name buffer program args)))
...
(defun lazyflymake-check-buffer ()
  ...
  (when lazyflymake-debug
    (message "lazyflymake-check-buffer called."))
  (cond ((null lazyflymake-timer)
        ...
        (t
         ...
         (when (and (< (buffer-size) lazyflymake-check-buffer-max))
           ...
           (when lazyflymake-debug
             (message "Flymake syntax checking ..."))))))
...
(defun lazyflymake--extract-err (output idx)
  ...
  (cond (idx (match-string idx output))
        (t "nil")))
...
(defun lazyflymake--legacy-info-at-point ()
  ...
  (cond
   ...
   (t
    (when-let ((ov (lazyflymake-overlay-at-point)))
    ...
    (cl-find-if (lambda (ov)
                  (and (< (overlay-start ov) (overlay-end ov))
                       (memq (overlay-get ov 'category)
                             '(flymake-note flymake-error flymake-warning))))
