...
Version 2018-06-04"
  (interactive)
  (let (($p (point)))
    (if (or (equal $p (line-beginning-position))
...
(progn
  ...
  (dotimes ($x (- (length xah-brackets) 1))
	(pcase (% $x 2)
	  (0 (push (char-to-string (elt xah-brackets $x))
               xah-left-brackets))
	  (1 (push (char-to-string (elt xah-brackets $x))
            xah-right-brackets))))
  (setq xah-left-brackets (reverse xah-left-brackets)
		xah-right-brackets (reverse xah-right-brackets)))

(defvar xah-punctuation-regex "[!\?\"\.,`'#$%&*+:;=@^|~]+"
      ...
      (save-restriction
        ...
        (let ((case-fold-search nil))
          ...
          (when (memq major-mode
					  '(xah-html-mode html-mode sgml-mode nxml-mode xml-mode mhtml-mode))
            (goto-char (point-min))
            (while
...
Version 2018-04-02"
  (interactive)
  (let* (($p3 (progn (skip-chars-backward "\n")
					 (point)))
		 ($p4 (progn (skip-chars-forward "\n")
					 (point))))
    (delete-region $p3 $p4)))
...
Version 2019-06-13"
  (interactive)
  (let* ((p1 (progn (skip-chars-forward " \t　")
					(point)))
    ...
    (cond
     ...
     ((> $eol-count 2)
      (if $space-neighbor-p
        ...
        (insert "\n")))
     (t (message "nothing done. logic error 40873. shouldn't reach here" )))))
...
Version 2017-01-17"
  (cl-flet ((aux-insert (beg end padding)
						(goto-char end)
						(insert @right-bracket)
						(goto-char beg)
						(insert @left-bracket)
						(goto-char (+ end padding))))
	(if (use-region-p)
	  ...
	  (cond
	   ...
	   ((eq @wrap-method 'block)
		(let (($p1 (save-excursion
		  ...
		  (aux-insert
		   $p1 $p2
		   (length @left-bracket))))
	   ((and ;;  do line. line must contain space
	   ...
	   (t ;; wrap around “word”. basically, want all alphanumeric, plus hyphen and underscore, but don't want space or punctuations. Also want chinese chars
...
;; (defmacro xah-define-delim-insert-fn (fn-name left-delim right-delim)
;;   `(defun ,fn-name ()
;; 	 (interactive)
;; 	 (xah-insert-bracket-pair ,left-delim ,right-delim)))
...
Version 2020-11-24"
  (interactive)
  (let (($skipChars "^\"`<>(){}[]“”‘’‹›«»「」『』【】〖〗《》〈〉〔〕（）〘〙")
        ($p1 (progn (skip-chars-backward $skipChars)
					(point))))
...
Version 2016-06-18"
  (interactive)
  (not (or (string= "*" (substring (buffer-name) 0 1))
		   (string= major-mode "dired-mode")
		   (string= major-mode "eww-mode"))))

(defun xah--repeat-20times-while (pred-fn body-fn)
  (funcall body-fn)
  (let ((i 0))
    (while (and (< i 20)
				(funcall pred-fn))
...
Version 2016-06-19"
  (interactive)
  (xah--repeat-20times-while (lambda () (not (xah-user-buffer-q)))
							 'next-buffer))
...
Version 2016-06-19"
  (interactive)
  (xah--repeat-20times-while
   (lambda () (not (string-equal "*" (substring (buffer-name) 0 1))))
   'next-buffer))
...
Version 2018-10-12"
  ...
  (let* (
		 ...
		 ($cmdStr (if current-prefix-arg
					  (format "%s build \"%s\" " $progName $fname)
					(format "%s run \"%s\" &" $progName $fname))))
  ...
  (let (($outBuffer "*xah-run output*")
	...
	(let* (($fname (buffer-file-name))
      ...
      (cond
       ...
       ((string-equal $fSuffix "go") (xah-run-current-go-file))
...
(defun xah--get-boundary (fn)
  (if (use-region-p)
      (funcall fn (region-beginning) (region-end))
	(fucall fn (point-min) (point-max))))
